...
function hitungRating(kriteriaId) {
  const nilaiInput = document.getElementById(`nilai_input_${kriteriaId}`).value;
  const ratingHidden = document.getElementById(`rating_${kriteriaId}`);
  const ratingDisplay = document.getElementById(`rating_display_${kriteriaId}`);
  const ratingText = document.getElementById(`rating_text_${kriteriaId}`);
  const subKriteriaInfo = document.getElementById(`sub_kriteria_info_${kriteriaId}`);

  // Reset jika nilai kosong
  if (!nilaiInput) {
    ratingHidden.value = '';
    ratingDisplay.textContent = '-';
    ratingText.textContent = 'Menunggu input nilai...';
    subKriteriaInfo.innerHTML = '';
    return;
  }

  // Ambil sub_kriteria untuk kriteria ini
  const subKriterias = subKriteriaBykriteria[kriteriaId] || [];

  if (subKriterias.length === 0) {
    ratingHidden.value = '';
    ratingDisplay.textContent = '-';
    ratingText.textContent = 'Tidak ada sub-kriteria';
  ...
  if (foundRating !== null && foundSub) {
    ...
    ratingText.textContent = `Rating: ${foundRating}★`;
    subKriteriaInfo.innerHTML =
      `<strong class="text-blue-600">${foundSub.nama}</strong> → Rating: <strong>${foundRating}</strong>`;
  } else {
    ratingHidden.value = '';
    ratingDisplay.textContent = '-';
    ratingText.textContent = 'Nilai tidak valid';
    subKriteriaInfo.innerHTML = '<span class="text-red-600">⚠ Nilai tidak sesuai dengan range sub-kriteria</span>';
  }
}
