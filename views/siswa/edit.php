...
// Placeholder for student data to be edited
$student_id = $_GET['id'] ?? 1; // Default to 1 for example
$student = [
    'id' => $student_id,
    'nis' => '12345',
...
<div class="flex-1 p-8">
    ...
    <div class="bg-white rounded-lg shadow-lg p-6">
        <form action="../../functions/func.php?action=update" method="POST">
            <input type="hidden" name="id" value="<?= $student['id'] ?>">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- NIS -->
                <div class="mb-4">
                    <label for="nis" class="block text-gray-700 font-semibold mb-2">NIS</label>
                    <input type="text" id="nis" name="nis" class="w-full px-4 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?= htmlspecialchars($student['nis']) ?>" required>
                </div>

                <!-- Nama Lengkap -->
                <div class="mb-4">
                    <label for="nama_lengkap" class="block text-gray-700 font-semibold mb-2">Nama Lengkap</label>
                    <input type="text" id="nama_lengkap" name="nama_lengkap" class="w-full px-4 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?= htmlspecialchars($student['nama_lengkap']) ?>" required>
                </div>

                <!-- Alamat -->
                <div class="mb-4 md:col-span-2">
                    <label for="alamat" class="block text-gray-700 font-semibold mb-2">Alamat</label>
                    <textarea id="alamat" name="alamat" rows="4" class="w-full px-4 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required><?= htmlspecialchars($student['alamat']) ?></textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select id="id_kelas" name="id_kelas" class="w-full px-4 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" required>
                        ...
                        <?php foreach($classes as $class): ?>
                            <option value="<?= $class['id_kelas'] ?>" <?= ($student['id_kelas'] == $class['id_kelas']) ? 'selected' : '' ?>>
